...
import { approveReschedule, rejectReschedule } from "../actions/decide-reschedule";
...
export function RescheduleRequestRow({
  ...
  request: ReschedulingRequestItem;
  studentName: string;
}) {
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="text-xs text-muted-foreground">
              De {formatDateTime(request.previousInterval.startsAt)} para{" "}
        ...
        {rejecting ? (
          <div className="space-y-2">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button
                ...
                size="sm"
                disabled={pending}
                onClick={() => startTransition(async () => void (await rejectReschedule(request.id, reason || undefined)))}
              >
                Confirmar recusa
              </Button>
            </div>
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              size="sm"
              disabled={pending}
              onClick={() => startTransition(async () => void (await approveReschedule(request.id)))}
            >
              <Check />
              Aprovar
            </Button>
