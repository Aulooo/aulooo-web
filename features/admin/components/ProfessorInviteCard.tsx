...
import { Check, Copy, ShieldPlus } from "lucide-react";
import { QRCodeSVG } from "qrcode.react";
import { Button } from "@/shared/components/ui/button";
import { Card, CardContent } from "@/shared/components/ui/card";
import { formatRelative } from "@/shared/lib/format";
import { generateProfessorEntryCode } from "../actions/generate-professor-entry-code";
import type { GenerateEntryCodeState } from "../actions/generate-professor-entry-code";

/** Gera o convite de professor (30 min) + link e QR prontos pra compartilhar. */
export function ProfessorInviteCard() {
  ...
  function handleGenerateCode() {
    startTransition(async () => {
      setCopied(null);
      const result = await generateProfessorEntryCode();
      setEntryCode(result);
    });
  }

  function signUpLink(code: string): string {
    return `${window.location.origin}/sign-up/professor?code=${code}`;
  }
    ...
    <Card>
      <CardContent className="space-y-3">
        ...
        {entryCode?.ok && entryCode.code ? (
          ...
          <p className="text-xs text-muted-foreground">
            Gere um código (ou QR) pra compartilhar com o novo professor — vale por 30 minutos.
          </p>
        )}
      </CardContent>
    </Card>
